<template>
  <div class="mod-dnf__role">
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="头像" header-align="center" align="center" width="80">
...
<!--      <el-table-column prop="movementSpeed" label="移动速度" sortable="custom" header-align="center" align="center"></el-table-column>-->
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          ...
          <el-button v-if="state.hasPermission('dnf:character:update')" type="primary" link @click="uploadAvatarHandle(scope.row.id)">上传头像 </el-button>
    ...
    <upload-avatar v-if="state.uploadVisible" ref="uploadRef" @refreshDataList="state.getDataList"></upload-avatar>
  </div>
</template>
...
import UploadAvatar from "@/views/dnf/upload-avatar.vue"; // 引入 Element UI 的消息提示组件
...
const uploadAvatarHandle = (id: number) => {
  state.uploadVisible = true;
  nextTick(() => {
    uploadRef.value.init(id);
  });
};
...
const calculatePercentagesAndDamagePerFame = (list) => {
  ...
  list.forEach((row) => {
    // 避免除以零
    row.famePercent = baseFame > 0 ? Math.round((row.fame / baseFame) * 100) : 0;

    row.damagePercent = baseDamage > 0 ? Math.round((row.simulatedDamage / baseDamage) * 100) : 0;

    row.damagePerFame = row.fame > 0 ? Math.round(row.simulatedDamage / row.fame) : 0; // 新增：使用Math.round取整
  });
};
